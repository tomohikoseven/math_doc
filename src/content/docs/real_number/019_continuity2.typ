...
#theorem-box(title: [定理 1.6（実数の連続性）])[
  $angle.l B, B' angle.r$ を#link("/real_number/018_real_number_cut/")[実数の切断]とするとき、次のいずれか一方が必ず成り立つ。
...
=== 実数の切断は実数である．
　$angle.l B,B' angle.r$ は実数の切断であって，まだ何者なのかわからない．つまり，実数直線上にはすき間があって，それを埋める何かかもしれない．ただここでは#circ[①②] のいずれかであることを示すので，最大なり最小なりの実数になりえるものを構成する．実数の切断があり，切断箇所に示したい実数があると思う．よって，
$
A = B sect QQ, quad A' = B' sect QQ, quad alpha = angle.l A, A' angle.r
$
とおく．

　注意したいのは，$alpha = angle.l A, A' angle.r$ はまだ#link("/real_number/010_dedekind_cut/")[有理数の切断]かわかっていないことだ．具体的に言うと，$A$ に最大の有理数がないかわかっていない．なぜなら，それは実数の切断 $angle.l B, B' angle.r$ から構成したからだ．

　$ alpha = angle.l A, A' angle.r$ が実は実数であることを示す．

　$A$ に属する最大の有理数$a$があるとする．すると，$A = B sect QQ$ だったので，$a$ は $B$ に属する最大の有理数である．$a$ は実数でもあるから，$B$上で考えると，$a < rho quad rho in B$ なる実数$rho$ があるとすれば，#link("/real_number/014_consistency/")[定理1.4]より，$a < r < rho$ なる有理数$r$がとれる．（実数$rho$ は有理数とは限らない．無理数とすれば，$a < rho$ なるものは存在する．）つまり，$a < r$ なる有理数$r$がとれる．有理数$a$ より大きい$A$ に属する有理数$r$が存在したので，矛盾が生じた．つまり，$A$ に属する最大の有理数$a$は存在しない．

　実数の切断$angle.l B, B' angle.r$の切断面が有理数であり，それが$B$に属するとき，上の論理は成り立たない．$a < rho quad rho in B$なる実数$rho$はないからである．なので，このときは切断面の有理数は$B'$に属すると考える（つまり，切断面が有理数なら，この有理数は$B'$に属するものと初めから考える）．実数の切断の定義上も問題ない．すると，$A$に属する最大の有理数$a$は切断面の点ではなくなり，上の論理が成り立ち，$A$に属する最大の有理数$a$は存在しない．

　$A$ に属する最大の有理数はないものとする．すると$alpha = angle.l A, A' angle.r$は有理数の切断であり，実数である．
...
　$alpha in B$とする．このとき，$alpha$は$B$の最大である．なぜなら，もし$alpha$が$B$の最大でないとすると，$alpha < rho$なる$rho in B$が存在する．この実数$rho$に対して，$alpha < r < rho$なる有理数$r in B$がとれる．このとき，$r in A$である．しかし，#link("/real_number/013_theorem1-3/")[定理1.3]より，$alpha = angle.l A, A' angle.r$において，$alpha < r$なる有理数$r$は$r in A'$のはず．矛盾が生じた．したがって，$alpha$は$B$の最大である．
