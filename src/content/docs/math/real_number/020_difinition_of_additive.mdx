...
description: 有理数の加法の定義と、実数の加法を有理数の切断を用いて定義する方法を解説します。補題1.1の主張とその証明の考え方も詳しく説明します。
...
<Card title="定義1.3">
　２つの実数$alpha = angle.l A, A^prime angle.r, beta = angle.l B, B^prime angle.r$が任意に与えられたとき，$alpha "と" beta$の和を
$$
alpha + beta = sigma, sigma = angle.l S, S^prime angle.r, S = A + B
$$
と定義する．
</Card>
...
t &= b -((a + b - r))/2
$$
このとき，$s + t = r$であり，$r < a + b$から$display(((a + b - r))/2 > 0)$から$s > a$および$r > b$がわかる．
...
### $angle.l S, S^prime angle.r$は有理数の切断である．
　$sigma = angle.l S, S^prime angle.r$と定義したが，$angle.l S, S^prime angle.r$が[有理数の切断](/math/real_number/010_dedekind_cut/)かどうかはまだ示されていない．
...
2. 有理数の切断の[条件3](/math/real_number/010_dedekind_cut/#1と同値な条件)を満たすこと．
...
　以上により，$angle.l S, S^prime angle.r$は有理数の切断であることが示せた．
