...
description: Astro/Starlight環境において、Typstの強力なダイアグラム描画ライブラリであるFletcherを導入し、D2の代替としてより美しく柔軟な関係図やフローチャートを描画するための実践的な設定ガイドです。
...
現在は以下の通り、Typst の公式パッケージマネージャを直接呼び出すだけで（`@preview/fletcher:0.5.8`など）、WASM 依存の最新版も含めて **完全かつ安定して** 動作します。
...
Fletcher をローカルの `libs/fletcher` に配置し、MDX 内の `$$` ブロックで呼び出すことで、D2 などを完全に廃止し、Typst の数式表現と調和した美しい描画環境を構築できる。
...
<Aside type="note" title="Q. なぜ D2 から Fletcher へ移行するのか？">
**A**: D2 は外部ツールや別エンジンでの描画に依存するため、数式フォントの統一や細かな位置・デザイン調整が困難であった。**Fletcher** を使用すれば、Typst のネイティブな描画能力を利用して、フォント・グリッド・線の太さ・余白に至るまで、数学ドキュメントと完全に一致した美しい図表を出力できるからである。
</Aside>
...
1.  **ライブラリのローカル配置**
    ...
    <LinkCard
      ...
      description="パッチ適用済みの fletcher ソースコードを公開している。ルートディレクトリの libs フォルダを参照のこと。"
